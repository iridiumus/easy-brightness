...
static int cmd_get(uint8_t vcp_code, const char *field_name) {
  ...
  for (int i = 0; i < dlist->ct; i++) {
    ...
    if (i > 0)
      printf(",");
    printf("{\"bus\":%d,\"serial\":\"%s\",\"%s\":%d}", di->path.path.i2c_busno,
           di->sn, field_name, value);
  }
  printf("]\n");
...
static int cmd_set(uint8_t vcp_code, const char *field_name, int target) {
  ...
  for (int i = 0; i < dlist->ct; i++) {
    ...
    rc = ddca_open_display2(di->dref, true, &dh);
    if (rc != 0) {
      printf("{\"bus\":%d,\"serial\":\"%s\",\"ok\":false,\"%s\":-1}", bus,
             serial, field_name);
      all_ok = 0;
      continue;
    }
    ...
    if (before == target) {
      printf("{\"bus\":%d,\"serial\":\"%s\",\"ok\":true,\"%s\":%d}", bus,
             serial, field_name, target);
      ddca_close_display(dh);
      continue;
    }
    ...
    printf("{\"bus\":%d,\"serial\":\"%s\",\"ok\":%s,\"%s\":%d}", bus, serial,
           ok ? "true" : "false", field_name, after);
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    fprintf(stderr, "Usage: easy-brightness-helper "
                    "<detect|get|set|get-blue|set-blue> [value]\n");
    return 1;
  }
